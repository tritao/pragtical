...
static const char *init_code =
  ...
  "  dofile(start_file)\n"
  "  if NATIVE_API_VERSION ~= system.api_version then\n"
  "    error(string.format('Pragtical native API mismatch (binary: %s, data: %s). Rebuild or reinstall Pragtical so its executable and data files match.', tostring(system.api_version), tostring(NATIVE_API_VERSION)), 0)\n"
  "  end\n"
