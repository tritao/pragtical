-- A deliberately small MessagePack codec for local control messages.
-- Control messages contain structured data and integers; terminal byte
...
local function i64(value)
  ...
  local bytes = {}
  for index = 8, 1, -1 do
    local high = math.floor(magnitude / 256)
    bytes[index] = magnitude - high * 256
    magnitude = high
  end
  local carry = 1
  for index = 8, 1, -1 do
    local inverted = 255 - bytes[index] + carry
    if inverted >= 256 then
      bytes[index], carry = inverted - 256, 1
    else
      bytes[index], carry = inverted, 0
    end
  end
  for index = 1, 8 do bytes[index] = byte(bytes[index]) end
  return table.concat(bytes)
end
...
local function encode_value(value, depth, seen, state)
  if depth > MAX_DEPTH then error("MessagePack nesting limit exceeded") end
  state.items = state.items + 1
  if state.items > MAX_ITEMS then error("MessagePack item limit exceeded") end
  ...
  if kind == "number" then
    if value ~= math.floor(value) then
      error("MessagePack values must use integer numbers")
    end
    if value >= 0 then
      ...
      if value <= MAX_SAFE_INTEGER then return "\xcf" .. u64(value) end
    else
      ...
      if value >= -MAX_SAFE_INTEGER then return "\xd3" .. i64(value) end
    end
    error("MessagePack integer is outside the safe range")
  end
  ...
  if is_array then
    if count < 16 then
      ...
      for index = 1, count do
        result[#result + 1] = encode_value(value[index], depth + 1, seen, state)
      end
      seen[value] = nil
  ...
  for key in pairs(value) do
    if type(key) ~= "string" then
      error("MessagePack maps require string keys")
    end
    keys[#keys + 1] = key
  end
  table.sort(keys)
  ...
  for _, key in ipairs(keys) do
    result[#result + 1] = encode_value(key, depth + 1, seen, state)
    result[#result + 1] = encode_value(value[key], depth + 1, seen, state)
  end
  seen[value] = nil
  return table.concat(result)
end
...
function msgpack.encode(value)
  return encode_value(value, 0, {}, { items = 0 })
end
...
local function read_u64(data, position)
  ...
  if value > MAX_SAFE_INTEGER then error("MessagePack integer is outside the safe range") end
  return value, position + 8
end

local function read_i64(data, position)
  need(data, position, 8)
  local negative = data:byte(position) >= 128
  local bytes = {}
  for index = 1, 8 do bytes[index] = data:byte(position + index - 1) end
  if negative then
    local carry = 1
    for index = 8, 1, -1 do
      local inverted = 255 - bytes[index] + carry
  ...
  local value = 0
  for index = 1, 8 do value = value * 256 + bytes[index] end
  if value > MAX_SAFE_INTEGER then error("MessagePack integer is outside the safe range") end
  return negative and -value or value, position + 8
end

local function decode_value(data, position, depth, state)
  if depth > MAX_DEPTH then error("MessagePack nesting limit exceeded") end
  state.items = state.items + 1
  if state.items > MAX_ITEMS then error("MessagePack item limit exceeded") end
  local marker
  ...
  if marker >= 0x90 and marker <= 0x9f then
    ...
    for index = 1, marker - 0x90 do
      result[index], position = decode_value(data, position, depth + 1, state)
    end
  ...
  if marker >= 0x80 and marker <= 0x8f then
    ...
    for _ = 1, marker - 0x80 do
      local key, value
      key, position = decode_value(data, position, depth + 1, state)
      value, position = decode_value(data, position, depth + 1, state)
...
function msgpack.decode(data, position)
  ...
  local value, next_position = decode_value(data, position or 1, 0, { items = 0 })
